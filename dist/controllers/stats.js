...
export const getDashboardStats = async (req, res, next) => {
    try {
        ...
        if (myCache.has("admin-stats")) {
            ...
            const [currentMonthProducts, lastMonthProducts, currentMonthUsers, lastMonthUsers, currentMonthOrders, lastMonthOrders, productCount, userCount, allOrders, lastSixMonthsOrders, categories, femaleUserCount, latestTransactions] = await Promise.all([
...
export const getPieChart = async (req, res, next) => {
    try {
        ...
        if (myCache.has("admin-pie-chart")) {
            pieChart = JSON.parse(myCache.get("admin-pie-chart"));
        }
        else {
            const allOrderPromise = Order.find({}).select(["total", "discount", "subtotal", "tax", "shippingCharges"]);
            const [processingOrder, shippedOrder, deliveredOrder, categories, productCount, productOutofStock, allOrders, allUser, adminUser, coustomerUser] = await Promise.all([
...
export const getBarChart = async (req, res, next) => {
    try {
        let barChart;
        if (myCache.has("admin-bar-chart")) {
            barChart = JSON.parse(myCache.get("admin-bar-chart"));
        }
        else {
            const today = new Date();
            const sixMonthsAgo = new Date();
...
export const getLineChart = async (req, res, next) => {
    try {
        let lineChart;
        if (myCache.has("admin-line-chart")) {
            lineChart = JSON.parse(myCache.get("admin-line-chart"));
        }
        else {
            const today = new Date();
            const twelveMonthsAgo = new Date();
